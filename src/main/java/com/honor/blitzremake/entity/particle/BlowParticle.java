...
public class BlowParticle extends Entity {
	...
	public BlowParticle(float x, float y, float life, boolean img) {
		...
		if (img) {
			...
			if (tex == 0)
				texture = Texture.blow1;
			if (tex == 1)
				texture = Texture.blow2;
			if (tex == 2)
				texture = Texture.blow3;
			if (tex == 3)
				texture = Texture.blow4;
		}
	}
